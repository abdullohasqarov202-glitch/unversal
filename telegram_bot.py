        ...
        try:
            ...
            if warn_count >= MAX_WARN:
                ...
                bot.send_message(chat_id,
                                 f"⚠️ {message.from_user.first_name}, reklama yoki spam o‘chirildi! "
                                 f"({warn_count}/{MAX_WARN})")
...
def warn_user(message):
    if not message.reply_to_message:
        bot.reply_to(message, "❌ Javob sifatida xabarni tanlang.")
        return
    user = message.reply_to_message.from_user
    chat_id = message.chat.id
    warnings.setdefault(chat_id, {}).setdefault(user.id, 0)
...
def start(message):
    ...
    bot.send_message(
        ...
        f"Botni guruhingizga qo‘shish uchun tugmani bosing:",
        reply_markup=markup
    )
...
if __name__ == "__main__":
    ...
    if WEBHOOK_URL:
        ...
        print(f"✅ Webhook o‘rnatildi: {WEBHOOK_URL}/{TELEGRAM_TOKEN}")
        port = int(os.environ.get("PORT", 5000))
        app.run(host="0.0.0.0", port=port)
    else:
        print("⚠️ WEBHOOK_URL topilmadi, bot polling orqali ishlaydi...")
